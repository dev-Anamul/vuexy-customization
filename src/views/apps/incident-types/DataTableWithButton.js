...
import AddIncidentTypeModal from "./AddIncidentsTypeModal";
...
const DataTableWithButtons = ({ data }) => {
  ...
  const handleFilter = (e) => {
    ...
    if (value.length) {
      updatedData = data.filter((item) => {
        const startsWith =
          item.incidentTypeName.toLowerCase().startsWith(value.toLowerCase()) ||
        ...
        const includes =
          item.incidentTypeName.toLowerCase().includes(value.toLowerCase()) ||
  ...
  return (
    <Fragment>
      ...
      <AddIncidentTypeModal open={modal} setOpen={setModal} />
    </Fragment>
  );
};
